...
module.exports = {
  ...
  update: async (req, res) => {
    try {
      ...
      if (!existingBook) return res.status(404).json({ message: "Livro não encontrado" });

      if (existingBook.userId !== req.user.id) {
        return res.status(403).json({ message: "Acesso negado: livro de outro usuário" });
      }

    
      if (existingBook.status === "LIDO") {
        return res.status(400).json({ message: "Livros com status 'LIDO' não podem ser editados" });
      }
